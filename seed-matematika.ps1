# SSL certificate check ni o'chirish
add-type @"
using System.Net;
using System.Security.Cryptography.X509Certificates;
public class TrustAllCertsPolicy : ICertificatePolicy {
    public bool CheckValidationResult(ServicePoint srvPoint, X509Certificate certificate, WebRequest request, int certificateProblem) {
        return true;
    }
}
"@
[System.Net.ServicePointManager]::CertificatePolicy = New-Object TrustAllCertsPolicy

$token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..kfJJjewcuMlyroXqnQiX2tD6tdZzWBJ7Mg99ZrGcpJY"

$headers = @{
    "Authorization" = "Bearer $token"
    "Content-Type" = "application/json"
}

$baseUrl = "https://localhost:5001"

$subjectId = "019b5946-1a9a-782c-8de4-42e9636ec2fa"
$subjectName = "Matematika"

Write-Host "Matematika fani uchun mavzu tekshirilmoqda..." -ForegroundColor Cyan

# Mavzu olish
try {
    $response = Invoke-RestMethod -Uri "$baseUrl/api/Topic/paged?SubjectId=$subjectId&PageNumber=1&PageSize=10&Search=" -Method Get -Headers $headers
    Write-Host "Mavzular javobi:" -ForegroundColor Yellow
    $response | ConvertTo-Json -Depth 5
    $topics = $response.Result.Values
    if ($topics -and $topics.Count -gt 0) {
        $topicId = $topics[0].Id
        Write-Host "Mavzu topildi: $topicId" -ForegroundColor Green
    }
} catch {
    Write-Host "Mavzu olishda xatolik: $($_.Exception.Message)" -ForegroundColor Red
}

# Mavzu yaratish
Write-Host "`nYangi mavzu yaratilmoqda..." -ForegroundColor Cyan
$topicBody = @{ TopicName = "Matematika - Umumiy"; SubjectId = $subjectId } | ConvertTo-Json
Write-Host "Body: $topicBody"
try {
    $response = Invoke-RestMethod -Uri "$baseUrl/api/Topic/create%20-%20topic" -Method Post -Headers $headers -Body $topicBody
    Write-Host "Mavzu yaratildi!" -ForegroundColor Green
    $response | ConvertTo-Json -Depth 5
    $topicId = $response.Result.Id
} catch {
    Write-Host "Mavzu yaratishda xatolik: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "Response: $($_.Exception.Response)" -ForegroundColor Red
}

if (-not $topicId) {
    Write-Host "Mavzu ID topilmadi, chiqish..." -ForegroundColor Red
    exit
}

Write-Host "`nSavollar qo'shilmoqda..." -ForegroundColor Cyan

# Savol yaratish funksiyasi
function Create-Question($subjectId, $topicId, $questionText, $level, $answers) {
    $boundary = [System.Guid]::NewGuid().ToString()
    $LF = "`r`n"

    $bodyLines = @()
    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"QuestionText`"$LF"
    $bodyLines += $questionText

    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"TopicId`"$LF"
    $bodyLines += $topicId

    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"SubjectId`"$LF"
    $bodyLines += $subjectId

    $bodyLines += "--$boundary"
    $bodyLines += "Content-Disposition: form-data; name=`"Level`"$LF"
    $bodyLines += $level

    for ($i = 0; $i -lt $answers.Count; $i++) {
        $bodyLines += "--$boundary"
        $bodyLines += "Content-Disposition: form-data; name=`"Answers[$i].Text`"$LF"
        $bodyLines += $answers[$i].Text

        $bodyLines += "--$boundary"
        $bodyLines += "Content-Disposition: form-data; name=`"Answers[$i].IsCorrect`"$LF"
        $bodyLines += $answers[$i].IsCorrect.ToString().ToLower()
    }

    $bodyLines += "--$boundary--"
    $body = $bodyLines -join $LF

    $multipartHeaders = @{
        "Authorization" = "Bearer $token"
        "Content-Type" = "multipart/form-data; boundary=$boundary"
    }

    try {
        $response = Invoke-RestMethod -Uri "$baseUrl/api/QuestionAnswer" -Method Post -Headers $multipartHeaders -Body $body
        return $true
    } catch {
        return $false
    }
}

$created = 0
$errors = 0

# Easy: 50 ta
for ($i = 1; $i -le 50; $i++) {
    $answers = @(
        @{ Text = "To'g'ri javob"; IsCorrect = $true }
        @{ Text = "Noto'g'ri A"; IsCorrect = $false }
        @{ Text = "Noto'g'ri B"; IsCorrect = $false }
        @{ Text = "Noto'g'ri C"; IsCorrect = $false }
    )
    if (Create-Question -subjectId $subjectId -topicId $topicId -questionText "Matematika - Oson savol #$i" -level 0 -answers $answers) {
        $created++
    } else { $errors++ }
    Write-Host "`r  Easy: $i/50" -NoNewline
}
Write-Host " OK" -ForegroundColor Green

# Medium: 100 ta
for ($i = 1; $i -le 100; $i++) {
    $answers = @(
        @{ Text = "To'g'ri javob"; IsCorrect = $true }
        @{ Text = "Noto'g'ri A"; IsCorrect = $false }
        @{ Text = "Noto'g'ri B"; IsCorrect = $false }
        @{ Text = "Noto'g'ri C"; IsCorrect = $false }
    )
    if (Create-Question -subjectId $subjectId -topicId $topicId -questionText "Matematika - O'rta savol #$i" -level 1 -answers $answers) {
        $created++
    } else { $errors++ }
    Write-Host "`r  Medium: $i/100" -NoNewline
}
Write-Host " OK" -ForegroundColor Green

# Hard: 50 ta
for ($i = 1; $i -le 50; $i++) {
    $answers = @(
        @{ Text = "To'g'ri javob"; IsCorrect = $true }
        @{ Text = "Noto'g'ri A"; IsCorrect = $false }
        @{ Text = "Noto'g'ri B"; IsCorrect = $false }
        @{ Text = "Noto'g'ri C"; IsCorrect = $false }
    )
    if (Create-Question -subjectId $subjectId -topicId $topicId -questionText "Matematika - Qiyin savol #$i" -level 2 -answers $answers) {
        $created++
    } else { $errors++ }
    Write-Host "`r  Hard: $i/50" -NoNewline
}
Write-Host " OK" -ForegroundColor Green

Write-Host "`nJami: $created ta qo'shildi, $errors ta xatolik" -ForegroundColor Cyan
