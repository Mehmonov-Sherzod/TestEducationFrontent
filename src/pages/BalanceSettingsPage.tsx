...
export const BalanceSettingsPage = () => {
  ...
  const handleUpdate = async () => {
    ...
    setIsSaving(true)
    try {
      const response = await fetch(`https://localhost:5001/api/BalanceTransaction?Id=${info?.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
            ...
            <div className="space-y-4">
              ...
              <div className={`p-4 rounded-xl ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2 mb-2">
                  <FiCreditCard className={`w-4 h-4 ${isDark ? 'text-cyan-400' : 'text-blue-600'}`} />
                  <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Karta raqami</p>
                </div>
              ...
              <div className={`p-4 rounded-xl ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
                <div className="flex items-center gap-2 mb-2">
                  <FaTelegram className={`w-4 h-4 ${isDark ? 'text-blue-400' : 'text-blue-500'}`} />
                  <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>Admin Telegram</p>
                </div>
