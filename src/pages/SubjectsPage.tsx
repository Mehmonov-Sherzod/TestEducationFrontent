      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-bold mb-2 flex flex-wrap items-center gap-2 sm:gap-3 ${isDark ? 'text-white' : 'text-gray-800'}`}>
            <FiBook className={isDark ? 'text-blue-400' : 'text-gray-600'} />
            <span>Subjects</span>
            {isSuperAdmin && (
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            <div className={`w-16 h-16 border-4 border-t-transparent rounded-full animate-spin mx-auto mb-4 ${isDark ? 'border-blue-500' : 'border-gray-400'}`}></div>
            <p className={`text-lg ${isDark ? 'text-blue-400' : 'text-gray-600'}`}>Loading subjects...</p>
          </div>
        </div>
      ) : subjects.length === 0 ? (
        <div className="flex items-center justify-center py-20">
        ...
        <>
          ...
          <div className={`hidden sm:block rounded-xl overflow-hidden border ${
            ...
            <div className={`grid grid-cols-12 gap-4 px-6 py-4 border-b ${
              ...
              <div className={`col-span-1 text-sm font-semibold ${isDark ? 'text-blue-400' : 'text-gray-600'}`}>
                #
              </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        isDark ? 'bg-blue-500/15 text-blue-400' : 'bg-gray-100 text-gray-600'
                      }`}>
                        #{(currentPage - 1) * pageSize + index + 1}
                      </span>
                    </div>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 mt-4 sm:mt-6 p-3 sm:p-4 rounded-xl border ${
              isDark ? 'bg-[#151515] border-gray-600/30' : 'bg-gray-50 border-gray-200 shadow-sm'
            }`}>
            ...
            >
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h2 className={`text-lg sm:text-2xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-700'}`}>
                  <FiBook className={`w-5 h-5 sm:w-6 sm:h-6 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
        ...
        {editingSubject && canManageSubjects && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h2 className={`text-lg sm:text-2xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-700'}`}>
                  <FiEdit2 className={`w-5 h-5 sm:w-6 sm:h-6 ${isDark ? 'text-blue-400' : 'text-blue-500'}`} />
                  Edit Subject
                </h2>
                <button
                  onClick={() => setEditingSubject(null)}
